(function (window) {
	...
	Swift.prototype.unbind = function (action, callback) {
		if (window.document.removeEventListener) this.each(function (i) {
			this.removeEventListener(action, callback);
		});
		else this.each(function (i) {
	...
	Swift.prototype.live = function (action, callback) {
		...
		swift(this.context).bind(action, function (event) {
			...
			for (var i = 0; i < matchedTargets.length; i++) {
				if (src == matchedTargets[i]) callback.call(src, event);
			}
		});
		return this;
	}
	...
	Swift.prototype.css = function () {
		...
		if (value === undefined) { // read
			...
			var elem = this[0];
			if (elem && elem.nodeType !== 3 && elem.nodeType !== 8 && elem.style) this.each(function () {
	...
	Swift.prototype.style = function (name) {
		if (this.length) {
			var name = swift.styleName(name);
			if (this[0].currentStyle) return this[0].currentStyle[swift.styleName(name)];
			else if (window.getComputedStyle) return document.defaultView.getComputedStyle(this[0], null).getPropertyValue(name);
		}
	}
	...
	window.$ = window.swift = swift = function (selector, ctx) {
		...
		if (type == "String") {
			...
			else var tags = ctx.querySelectorAll(selector);
	...
	swift.filter = function (items, callback) {
		...
		for (var i = 0; i < len; i++) {
			if (i in items) {
				...
				if (callback.call(val, i, items)) res.push(val);
			}
		}
		return res;
	}
